...
const GooeyNav = ({
  ...
  useEffect(() => {
    ...
    if (activeLi) {
      ...
      // Trigger particles whenever activeIndex changes
      if (filterRef.current) {
        setTimeout(() => {
          makeParticles(filterRef.current);
        }, 50);
  ...
  }, [activeIndex]); // This triggers whenever activeIndex changes

  // Add separate useEffect to sync with prop
  useEffect(() => {
    setActiveIndex(initialActiveIndex);
  }, [initialActiveIndex]);
    ...
    <>
      ...
      <div className="gooey-container" ref={containerRef}>
        <nav style={{ transform: "translate3d(0,0,0.01px)" }}>
          <ul
            ref={navRef}
            className="gooey-nav flex gap-6 list-none p-0 m-0 relative z-[3]"
            style={{
              color: "white",
